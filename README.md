...
> Caliper decided **73%** of its 51 cases without a human and committed **no unsafe error**. The
> single-prompt baseline decided **86%** of the same cases, with the same model and the same
...
Read that column with suspicion anyway. The key expects `ineligible` for 41 of its 51 cases, because
