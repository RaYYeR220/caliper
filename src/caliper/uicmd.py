...
What is hand-written is exactly two things, and both are things a language model would otherwise
have produced: the `CriteriaSet` below — one criterion per compile unit of NCT01131676, with the
protocol quoted verbatim — and the critic's severity and reason for each criterion it reviewed.
Everything else on screen is computed here and now: the units are planned by `plan_units`, the
back-translated English by `render_predicate`, the coverage by `coverage_report`, the downgrades by
...
The compilation is deliberately not flattering. Thirteen of these twenty-two criteria could not be
formalised at all and three more did not survive back-translation, which is what this protocol
actually looks like when you refuse to guess — and which is the case the interface exists to make.
"""
...
from caliper.uiexport import ScreeningRecord, write_ui_bundle
...
def _criteria(source_text: str) -> CriteriaSet:
...
_REVIEW: dict[str, tuple[Severity, str]] = {
    "INC-01": (
        "equivalent",
...
def _review(criteria_set: CriteriaSet) -> CriticReport:
    """The critic's report, with the rendered sentence and the coverage produced by real code.

    Only the severity and the reason come from the table above. `render_predicate` renders what was
    compared and `coverage_report` audits what the compiler left behind, both deterministically.
    """
    findings = tuple(
        Finding(
            criterion_id=criterion.id,
            severity=_REVIEW[criterion.id][0],
            reason=_REVIEW[criterion.id][1],
            rendered=render_predicate(criterion.predicate),
            quote=criterion.source_quote,
        )
        for criterion in criteria_set.criteria
        if criterion.id in _REVIEW
    )
    return CriticReport.from_coverage(findings, coverage_report(criteria_set))


def compile_fixture(criteria_text: str) -> CompiledTrial:
    """Assemble the compiled trial the interface reads, running every real stage over the fixture.
    ...
    criteria_set = _criteria(source_text)
    ...
    if problems:
        ...
        raise typer.BadParameter(
            "the fixture quotes do not match the protocol text: "
            + "; ".join(f"{p.criterion_id} ({p.reason})" for p in problems)
        )

    report = _review(criteria_set)
...
def screen_cohort(trial: CompiledTrial, as_of: date) -> list[ScreeningRecord]:
    """Screen every patient in the committed corpus against the compiled trial."""
    records = []
    title = corpus.load_trial(FIXTURE_NCT).title
    for patient_id in corpus.patient_ids():
        patient = corpus.load_patient(patient_id)
        result = screen(trial.criteria_set, patient, as_of, policy=AbsencePolicy.COVERAGE_GATED)
        screening = Screening(
            trial=trial, result=result, rationales=deterministic_rationales(result)
        )
        records.append(
            ScreeningRecord(screening=screening, patient=patient, trial_title=title)
        )
    return records
...
def demo(
    ...
    written = write_ui_bundle(records, root=root)

    report = compiled.critic_report
    assert report is not None  # `compile_fixture` always reviews
    console.print(
        f"{FIXTURE_NCT}  {len(compiled.criteria_set.criteria)} criteria, "
    ...
    table = Table("Decision", "Patients", "Criteria resolved", "Gaps a query would close", box=None)
    for decision in ("eligible", "needs_review", "ineligible"):
        rows = [r for r in records if r.screening.result.decision.value == decision]
        if not rows:
            continue
        results = [r.screening.result for r in rows]
        ...
        table.add_row(
            decision.replace("_", " "),
            str(len(rows)),
            f"{resolved:.1f} of {results[0].criteria_total}",
