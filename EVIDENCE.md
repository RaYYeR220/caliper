...
| Every unresolved criterion carries what would resolve it | VERIFIED | `ResolutionHint`, parametrized in `tests/test_evaluate.py` over all eleven predicate shapes the IR defines — including composites and the medication and procedure variants of presence, which an audit found the earlier four-case list silently excluded. A second test compares the list against `ir.Predicate` itself, so adding a predicate type without adding it there fails rather than quietly narrowing the invariant |
